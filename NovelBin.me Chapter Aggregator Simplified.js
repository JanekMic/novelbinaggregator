...
(function () {
  ...
  class ChapterExtractor {
    ...
    extractChapterContent(html, chapterUrl) {
      try {
        ...
        logger.error(`Failed to extract content from ${chapterUrl}`, {
          error: error.message,
        });
        throw error;
      }
    ...
    async processChapters(chapters, onProgress) {
      ...
      logger.info(
        `Starting to process ${chapters.length} chapters in batches of ${this.batchSize}`,
      );
      ...
      for (let i = 0; i < chapters.length; i += this.batchSize) {
        ...
        logger.info(
          `Processing batch ${Math.floor(i / this.batchSize) + 1}/${Math.ceil(chapters.length / this.batchSize)}`,
        );
  ...
  class UIController {
    ...
    isValidPage() {
      ...
      const validUrl =
        url.includes("novelbin.me") || url.includes("novelbin.com");
    ...
    refreshChapterList() {
      ...
      if (newCount !== oldCount) {
        this.showNotification(
          `📱 Found ${newCount} chapters (${newCount > oldCount ? "+" + (newCount - oldCount) : newCount - oldCount} from before)`,
          "success",
        );
      } else {
    ...
    handleRangeSelection() {
      ...
      if (
        fromValue < 1 ||
        toValue < 1 ||
        fromValue > this.chapters.length ||
        toValue > this.chapters.length
      ) {
        this.showNotification(
          `Please enter chapter numbers between 1 and ${this.chapters.length}`,
          "error",
        );
        return;
      }
      ...
      this.showNotification(
        `📋 Selected chapters ${fromValue} to ${toValue} (${count} chapters)`,
        "success",
      );
    }
    ...
    extractChapterList() {
      try {
        const chapterElements = document.querySelectorAll(
          ".list-chapter li a, .chapter-item a, .chapter-list a",
        );
        logger.info(`Found ${chapterElements.length} chapter elements`);

        this.chapters = Array.from(chapterElements)
          .map((el, index) => {
            const titleEl =
              el.querySelector(".chapter-title, span, .title") || el;
        ...
        logger.error("Failed to extract chapter list", {
          error: error.message,
        });
        this.chapters = [];
      }
    ...
    bindUIEvents() {
      // Header buttons
      document
        .getElementById("minimize-aggregator")
      ...
      document
        .getElementById("refresh-chapters")
      ...
      // Download controls
      document
        .getElementById("download-chapters")
        .addEventListener("click", (e) => {
          e.target.style.transform = "scale(0.95)";
          setTimeout(() => (e.target.style.transform = "scale(1)"), 100);
          this.startDownload();
    ...
    saveSettings() {
      const batchSize = parseInt(
        document.getElementById("setting-batch-size").value,
      );
      const baseDelay = parseInt(
        document.getElementById("setting-base-delay").value,
      );
      const maxRetries = parseInt(
        document.getElementById("setting-max-retries").value,
      );
      const enableLogging = document.getElementById(
        "setting-enable-logging",
      ).checked;
      const compactMode = document.getElementById(
    ...
    resetSettings() {
      ...
      document.getElementById("setting-batch-size").value =
        settingsManager.get("batchSize");
      document.getElementById("setting-base-delay").value =
        settingsManager.get("baseDelay");
      document.getElementById("setting-max-retries").value =
        settingsManager.get("maxRetries");
      document.getElementById("setting-enable-logging").checked =
        settingsManager.get("enableLogging");
      document.getElementById("setting-compact-mode").checked =
        settingsManager.get("compactMode");
    ...
    updateUI() {
      ...
      if (this.selectedRange) {
        ...
        logger.info(
          `UI updated for range selection: ${from}-${to} (${count} chapters)`,
        );
      } else {
      ...
      if (this.isProcessing) {
        ...
        statusBadge.style.background =
          "linear-gradient(45deg, #FF9800, #F57400)";
      } else {
    ...
    async startDownload() {
      ...
      try {
        const { results, failed, cancelled } =
          await this.extractor.processChapters(selectedChapters, (progress) =>
        ...
        if (cancelled) {
          ...
          if (failed.length > 0) {
            this.showNotification(
              `Downloaded ${results.length}/${selectedChapters.length} chapters. ${failed.length} failed.`,
              "error",
            );
          } else {
            const rangeText = this.selectedRange
              ...
              : "all chapters";
            this.showNotification(
              `Successfully downloaded ${rangeText} (${results.length} chapters)!`,
              "success",
            );
          }
        }
    ...
    generateFileName(chapters) {
      ...
      const contentHash = generateHash(
        chapters.map((ch) => ch.chapterTitle).join(""),
      ).slice(0, 6);
